...
TODAY = "2026-09-16"
TODAY_DISP = "2026년 9월 16일 (수)"

with open("/sessions/wonderful-zealous-turing/mnt/outputs/parsed_disclosures.json", encoding="utf-8") as f:
    parsed = json.load(f)
with open("/sessions/wonderful-zealous-turing/mnt/outputs/prices_all.json", encoding="utf-8") as f:
    prices = json.load(f)
with open("/sessions/wonderful-zealous-turing/mnt/outputs/company_info.json", encoding="utf-8") as f:
    company_info = json.load(f)
with open("/sessions/wonderful-zealous-turing/mnt/outputs/naver_finance.json", encoding="utf-8") as f:
    naver = json.load(f)

# 한글 큐레이션된 overrides (WebSearch + 사용자 지식 기반)
ENRICHED = {}
override_path = "/sessions/wonderful-zealous-turing/mnt/outputs/enriched_overrides.json"
if os.path.exists(override_path):
    with open(override_path, encoding="utf-8") as f:
        ENRICHED = json.load(f)

# Aggregates (cumulative)
AGG_PATH = "/sessions/wonderful-zealous-turing/mnt/outputs/daily_aggregates.json"
agg_data = {"by_date": {}}
...
DAILY_ANALYSES = {}
daily_path = f"/sessions/wonderful-zealous-turing/mnt/outputs/daily_analyses_2026-09-16.json"
if os.path.exists(daily_path):
    with open(daily_path, encoding='utf-8') as f:
        DAILY_ANALYSES = json.load(f)
    print(f'✓ Loaded daily_analyses_2026-09-16.json: {len(DAILY_ANALYSES)} companies')
...
def tipping_insight():
    if len(all_dates) < 2:
        return f"📊 오늘 (2026-09-16) 누적 데이터 시작. 7일치 누적 후부터 산업 모멘텀 추이가 의미있는 시그널을 제공합니다. 현재 상위 산업은 {', '.join([f'{k}({v})' for k,v in top_inds[:5]])}."
...
<title>AWAKE 전자공시 일일 리포트 — 2026-09-16</title>
...
<div style="font-size:14px; color:var(--c-mute); margin-bottom:10px;">2026-09-16 · 종가 yfinance · 종목명 클릭 시 상세 페이지로 이동 · <span id="idx-count-lbl">{len(DISCLOSURES)}건 표시</span></div>
...
<td style="text-align:center;"><button class="fav-star-idx" data-code="{html.escape(d["code"])}" data-name="{html.escape(d["company"])}" data-date="2026-09-16" data-signal="{d["signal_kind"]}" data-report="{html.escape(d["report"][:60])}" data-anchor="stock-{html.escape(d["code"])}-{d["id"]}" data-close="{int(d["price"]["close"]) if d.get("price") and d["price"].get("close") else 0}" data-chg="{round(d["chg_pct"],2) if d.get("chg_pct") is not None else 0}" onclick="toggleFavFromIdx(this,'{html.escape(d["code"])}')">☆</button></td>
...
var IDX_SS_KEY = 'awake_idx_state_2026-09-16';
...
var REPORT_DATE = '2026-09-16';  // 빌드 시 하드코딩 — 날짜별 즐겨찾기 격리용
...
<div class="stock-meta">
  <div class="co-block">
    <div style="display:flex;align-items:center;gap:10px;">
      ...
      <button id="fav-btn-{html.escape(code)}" class="fav-star-page" data-code="{html.escape(code)}" data-name="{html.escape(company)}" data-date="2026-09-16" data-signal="{first["signal_kind"]}" data-firstid="{first["id"]}" data-report="{html.escape(first["report"][:60])}" data-close="{int(p_data["close"]) if (p_data:=prices.get(code)) and p_data.get("close") else 0}" data-chg="{round(p_data["chg_pct"],2) if (p_data:=prices.get(code)) and p_data.get("chg_pct") is not None else 0}" onclick="toggleFavFromPage('{html.escape(code)}')" title="즐겨찾기">☆</button>
    </div>
...
out_path = "/sessions/wonderful-zealous-turing/mnt/outputs/AWAKE_v11.html"
